...
// #include "ftheboys.c"  deve estar dentro do make file.
...
int main ()
{
  ...
  for (int i = 0; i < tam; i++)
  {
    mundo.h[i]= cria heroi;
    mundo.m[i]= cria missao;
    mundo.b[i]= cria base;
  }

  *(*mundo)->heroi = adiciona_heroi(tam);
  /*aumenta nível do heroi*/

  *(*mundo)->heroi = aumento_de_nível(heroi,1)
