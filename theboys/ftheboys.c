/* Implementação das funções do projeto "The Boys" ftheboys.c*/
...
struct missao *adiciona_missao(struct mundo *w) {
    ...
    struct missao *m;
    m = cria_missao_aleatoria(w->NMissoes, w->TamanhoMundoX, w->TamanhoMundoY);
...
void chega(struct mundo *w, struct fprio_t *lef, struct evento *ev) {
    ...
    espera = false; /*estado original*/
    ocupacao = cjto_card(b->presentes);
    vaga = (ocupacao < b->Lotacao);
    fila_vazia = (fila_tamanho(b->Espera) == 0);

    /*Esperar ou Desistir*/
    if (vaga && fila_vazia) {/*se houver vaga e fila vazia*/
        espera = true; /*atualização de estado*/
    } 
    else {
        /*lógica para permanência*/
...
void desiste(struct mundo *w, struct fprio_t *lef, struct evento *ev) {
    ...
    int id_destino;
    /*escolha de base aleatória*/
    id_destino = aleat(0, w->NBases - 1);

    /*criação do evento viagem*/
    struct evento *viagem;
    viagem = cria_evento(w->relogio, EV_VIAJA, id_heroi, id_destino);
...
void espera(struct mundo *w, struct fprio_t *lef, struct evento *ev) {
    ...
    /*evento aviso*/
    struct evento *aviso;
    aviso = cria_evento(w->relogio, EV_AVISA, 0, id_base);
    if (aviso) 
...
void avisa(struct mundo *w, struct fprio_t *lef, struct evento *ev) {
    ...
    while (cjto_card(b->presentes) < b->Lotacao && fila_tamanho(b->Espera) > 0) {
        ...
        struct evento *entra;
        entra = cria_evento(w->relogio, EV_ENTRA, id_adm, id_base);
        if (entra) 
...
void entra(struct mundo *w, struct fprio_t *lef, struct evento *ev) {
    ...
    int tempo_saida;
    tempo_saida = w->relogio + tpb;

    /*printf muito longo, por isso quebrei em duas linhas*/
    printf("%6d: ENTRA HEROI %2d BASE %d (%2d/%2d) SAI %d\n",
        w->relogio, id_heroi, id_base, cjto_card(b->presentes), b->Lotacao, tempo_saida);

    /*criaçao do evento sai*/
    struct evento *sai;
    sai = cria_evento(tempo_saida, EV_SAI, id_heroi, id_base);
    if (sai) 
...
void evento_sai(int tempo, int id_heroi, int id_base, struct mundo *w, struct fprio_t *lef) {
    ...
    /*escolha da base aleatória*/
    int id_dest = aleat(0, w->NBases - 1);
    /*criação do evento viagem*/
    struct evento *viagem;
    viagem = cria_evento(tempo, EV_VIAJA, id_heroi, id_dest);
...
void evento_viaja(int tempo, int id_heroi, int id_destino, struct mundo *w, struct fprio_t *lef) {
    ...
    /*utilizei cast para trocar os tipos das variáveis corretamente*/
    ...
    /*Duração definida pela distância e velocidade do heroi.*/
    int duracao;
    ...
    int tempo_chegada;
    tempo_chegada = tempo + duracao;

    /*quebra de linha pra facilitar a compreensão.*/
    printf("%6d: VIAJA HEROI %2d BASE %d BASE %d DIST %d VEL %d CHEGA %d\n",
           tempo, id_heroi, id_origem, id_destino, dist, h->Velocidade, tempo_chegada);

    /*criamos o eventos chega*/
    struct evento *chega;
    chega = cria_evento(tempo_chegada, EV_CHEGA, id_heroi, id_destino);
    if (chega) 
        fprio_insere(lef, chega, EV_CHEGA, tempo_chegada);
}
void evento_morre(int tempo, int id_heroi, int id_base, struct mundo *w, struct fprio_t *lef) {
    ...
    cjto_retira(b->presentes, id_heroi); /*remove o heroi da base*/
    /*atualiza os estatus do heroi*/
    h->vivo = 0;
    h->Base = -1;

    printf("%6d: MORRE HEROI %2d BASE %d\n", tempo, id_heroi, id_base);

    /*Avisa porteiro (nova vaga)*/
    struct evento *aviso;
    aviso = cria_evento(tempo, EV_AVISA, 0, id_base);
...
void evento_missao(int tempo, int id_missao, struct mundo *w, struct fprio_t *lef) {
    ...
    m = w->M[id_missao];
    bmp = NULL;
    /*variáveis auxiliares declaradadas a qui para evitar erros*/
    int id_bmp;
    double min_dist_apta;
    int id_base_mais_prox_abs;
    double min_dist_abs;

    id_bmp = -1; /*escolha de uma não base*/
    min_dist_apta = 1e9;
    id_base_mais_prox_abs = -1; /*usado para o composto v*/
    min_dist_abs = 1e9;

    /*Tentativdas de missões*/
    m->total_tentativas++;
    ...
    for (int i = 0; i < w->NBases; i++) {
        ...
        double dy;
        double dist;
        dx = (double)(b->local_x - m->local_x);
        dy = (double)(b->local_y - m->local_y);
        dist =  sqrt(dx * dx + dy * dy);
                

        /* Base mais próxima absoluta (compoto V)*/
        if (dist < min_dist_abs) {
            min_dist_abs = dist;
            id_base_mais_prox_abs = i; /*troca do -1*/
        }
        ...
        /*Faz a união dos habilidades dos herois da base*/
        for (int k = 0; k < w->NHerois; k++) {
            struct heroi *h;
            h = w->H[k];
            /*confirma se heroi está vivo*/
            if (h && h->vivo && cjto_pertence(b->presentes, h->ID)) {
                
                struct cjto_t *aux;
                aux = cjto_uniao(h->Habilidades, uniao);
    ...
    if (id_bmp != -1) { 
        ...
        if (m->total_tentativas > w->max_tentativas) 
            w->max_tentativas = m->total_tentativas;
        if (m->total_tentativas < w->min_tentativas) 
            w->min_tentativas = m->total_tentativas;

    } else {
        /*falha = composto V*/
        int usar_v;
        /*condições para que possa utilizar o composto V*/
        usar_v = (w->NCompostosV > 0) && (tempo % 2500 == 0) && (id_base_mais_prox_abs != -1); 

        if (usar_v) {
            struct base *b_prox;
            b_prox = w->B[id_base_mais_prox_abs];
            w->NCompostosV--;
            b_prox->n_missoes++;
            ...
            /*agenda a morte do heroi de maior exp*/
            if (h_max) {
                struct evento *morre;
                morre = cria_evento(tempo, EV_MORRE, h_max->ID, b_prox->ID);
                if (morre) 
                    fprio_insere(lef, morre, EV_MORRE, tempo);
            }

            /*MISSÂO Completada logo aumento de xp.*/ 
            for (int k = 0; k < w->NHerois; k++) {
                struct heroi *h;
                h = w->H[k];
            ...
            /*reagendamento da mnissão*/
            printf("%6d: MISSAO %d IMPOSSIVEL\n", tempo, id_missao);

            struct evento *nova;
            nova = cria_evento(tempo + 24 * 60, EV_MISSAO, id_missao, 0); /*24 horas*/
...
void evento_fim(int tempo, struct mundo *w) {
    ...
    /*Estatística do Mundo*/
    for (int i = 0; i < w->NHerois; i++) {

        h = w->H[i];
        if (!h->vivo) 
            mortos++;
        /*quebra de linha para facilitar a compreensão*/
...
/*Estatísticas de missões*/
    ...
    /*Evitar disão por zero*/
    if (w->NMissoes > 0) {
        /* vezes 100 pois inteiro para decimal */
        perc_missoes = (w->missoes_cumpridas * 100.0) / w->NMissoes; 
    }
    
    printf("MISSOES CUMPRIDAS: %d/%d (%.1f%%)\n", 
           w->missoes_cumpridas, w->NMissoes, perc_missoes);


    /* 2. Cálculo da Média e Mínimo de Tentativas */
    double media_tentativas;
    media_tentativas = 0.0;
    
    /*Evitar disão por zero*/
    if (w->missoes_cumpridas > 0) {
        media_tentativas = (w->total_tentativas * 100.00) / w->missoes_cumpridas;
    }

    /* Ajusta o mínimo: se for o valor sentinela (999999), exibe 0*/
    int min_tentativas = w->min_tentativas;
    if (min_tentativas == 999999) {
        min_tentativas = 0;
    }
    
    printf("TENTATIVAS/MISSAO: MIN %d, MAX %d, MEDIA %.1f\n", 
           min_tentativas, w->max_tentativas, media_tentativas);


    /*Estatística da Taxa de Mortalidade*/
    double taxa_mortalidade;
    taxa_mortalidade = 0.0;
    
    if (w->NHerois > 0) {
        taxa_mortalidade = (mortos * 100.0) / w->NHerois;
    }
